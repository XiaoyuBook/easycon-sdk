...
- 已取代候选 HEAD：`10742b6f28ed17ab429c7e365d520a5f54cff543`
- 已取代软件实现：`eef1ed35c172fe31e805d634aa324170856e782f`
- 重新冻结的软件实现：`bbebaf458a0f2c0d60f3de6169f0eecfe8ef9fd0`
- 软件实现 tree：`020635276554be62d77ef26f8ab9f5bb8237e8ca`
- 完整审查范围：`202b2610ffc836f21951e18f69fa98b46d6efc90..bbebaf458a0f2c0d60f3de6169f0eecfe8ef9fd0`
- 最终候选 HEAD：包含本 ADR 的独立 `docs:重新冻结Phase 2B软件资格候选` 提交；其精确 SHA 由交接清单固定
- 上位目标：[ADR-0010](0010-phase-2b-qualification-evidence.md)

## 决策

将 `bbebaf458a0f2c0d60f3de6169f0eecfe8ef9fd0` 冻结为 **Phase 2B Qualification Software Candidate**，并以
包含本 ADR 的单独说明性提交作为最终候选 HEAD。旧候选 `10742b6f28ed17ab429c7e365d520a5f54cff543`
及其实现 `eef1ed35c172fe31e805d634aa324170856e782f` 已被取代。新候选已经具备 ADR-0010 要求的
...
根 conformance 门禁获准执行一次只读 Windows SetupAPI/registry 串口发现；它不打开设备、不写设备，也不形成
硬件资格证据。
...
| checkpoint | 磁盘 evidence 重验、完整 provenance/trust 一致性、Observed/Attested/Unverified/Failed/NotRun 五类、strict attestation、`Hardware Unverified` transaction | `eff496f`、`bbebaf4` |
...
初次审查固定 `9944dba` 为只读起点，逐提交和组合检查 `202b261..9944dba`，随后每个软件节点在完整门禁后形成
新基线；最终冻结实现为 `eef1ed3`，候选 HEAD 为 `10742b6`。本次复审从该候选继续，组合检查
`10742b6..bbebaf4` 与 `202b261..bbebaf4`；后一个范围共有 63 个线性提交、0 个 merge。审查始终区分可复现
bug、硬件未知、架构决定和未来加固，O-01/O-02/O-04、缺少目标设备和已登记的 nominal-directory
...
| Q-03 | P2 | checkpoint 只读取 completed document 的 `provenance.trusted` 布尔值；仅含 `{"trusted":true}` 的畸形 provenance 可被投影为 `Observed`。 | `incomplete_pass_provenance_is_failed_instead_of_observed` 固定复现；`bbebaf4` 严格解析完整 build/runtime provenance、字段格式及 trust 一致性。 |
| Q-04 | P2 | 测试策略链接到 tracked tree 外的根 `AGENTS.md`；不携带该本机规则文件的干净候选运行 Markdown gate 时链接失效。 | `850fea9` 保留规则说明但移除不可分发链接，干净 tracked tree 可独立校验。 |

Q-03/Q-04 分别形成独立修复提交；`10742b6..bbebaf4` 只修改测试策略说明和 checkpoint 实现/回归，不改变
production crate、根 workspace、release 输入或 `EasyCon/`。一名未参与发现或修复的新独立 reviewer 在修正后的
安全边界下复跑全部 13 项门禁并复审两个差异范围，没有发现新的直接相关、可复现且 in-scope 的 P0/P1/P2。
结论为 **MERGE READY FOR SOFTWARE CANDIDATE**。理论攻击、风格建议和不改变当前契约真实性/资源安全的加固
没有扩大本冻结范围。
...
- 根 workspace：157 个非文档测试通过，0 failed、0 ignored、0 filtered；独立 Runtime Loom 6/6 通过。
- hardware workspace：file-id 1、library 2、CLI/unit 154、artifact integration 4、checkpoint integration 2、
  qualification-status integration 1，共 164 个测试通过，0 failed、0 ignored、0 filtered。
- 规范：5 schemas、1 behavior、3 controller fixtures、9 scenarios、65 个 exact Rust tests。
- 实现基线 Markdown：145 个引用、33 个文件；repository guards 和最终 diff check 通过。
- `tests/hardware/Cargo.lock` SHA-256：
  `F4AF3D9A0122DDDB0EB815404CE2468D7EC03725BE1313026FFB7AC3EC1869E1`。

根 `cargo test --workspace --all-features` 包含 ADR-0009/exact conformance 固定的
`live_discovery_is_sorted_and_has_stable_identity`。该测试只调用 `discover_system_ports()` 进行一次只读 Windows
SetupAPI/registry 枚举、排序和 stable identity 字段检查；它不构造 `WindowsByteIoFactory`，不调用 `CreateFile`、
open 或 write，也不升级为硬件资格证据。未来是否迁移到显式 opt-in live suite 仅为非阻断测试隔离建议；实施前
必须另行重开 ADR-0009 与 conformance 设计，不能混入本冻结修复。

干净 `bbebaf4` 上另运行一次只含空 synthetic runs root 的 `checkpoint`。它没有调用 discovery、Harness 或
operator observation，输出固定为 `completed/unverified/2` 和 `Hardware Unverified`，且 build provenance 为：

- commit `bbebaf458a0f2c0d60f3de6169f0eecfe8ef9fd0`；
- tree `020635276554be62d77ef26f8ab9f5bb8237e8ca`；
- `tracked_dirty=false`、`untracked_present=false`、`trusted=true`；
- tracked source SHA-256 `316EAD646E3DA0B47AC34E65F203D39EC6A21D54F6F846939ED31569EEAD9C9D`；
- Cargo.lock SHA-256 `F4AF3D9A0122DDDB0EB815404CE2468D7EC03725BE1313026FFB7AC3EC1869E1`；
- executable SHA-256 `C7AB012A4B1FB21D1A4CEB7CA214983329FD88F0C1285D4ADB35E305712DAAD7`。
...
`bbebaf4`；但实际运行必须记录其真实 build commit。任何 executable-affecting descendant 都不能继续冒充本候选。
