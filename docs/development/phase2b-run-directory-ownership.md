...
“永久保留”只表示 qualification runner 从不删除或回收这些名字，不是 ACL、数字签名或永久防篡改承诺。当前
进程退出后，拥有目录写权限的外部主体仍可修改或删除文件；后续 manifest/checkpoint 必须从磁盘重算 hash，
交接包再以外层 SHA-256 固定证据。
...
- `Published`：auxiliary final 先发布、primary final 最后发布，随后通过 post-publication 验证；tombstone 和 staging
  都保留。
...
Windows share mode 与 retained handle 固定的是当前进程存活期间的 tombstone 对象：标准文件 API 无法在 handle
...
5. 按固定顺序用同目录 `hard_link(staging, final)` 发布已登记 auxiliary，最后发布 primary JSON。任一 final
   已存在或 filesystem 不支持 hard link 都保守失败，不覆盖、不删除、不 rollback 已发布的 earlier link。
6. handles 保持打开，再次枚举 post-publication allowlist，并从每个 final path 读取 exact bytes；必须恰好包含
   tombstone、全部 retained staging 和已经发布的 final。发现污染、缺失或 byte mismatch 时返回 artifact failure，
   状态为 `Incomplete`，不删除已经发布的文件。
7. 验证成功后返回 primary final relative path，随后 owner drop 才关闭 handles。tombstone、staging 和 final
   都永久保留且 reservation bytes 不改变。
...
污染边界。transaction 存活期间，外部 replacement 应由 Windows share-mode 拒绝；若 retained object 验证仍失败，
runner fail closed，保留磁盘内容且不尝试“修复”或删除路径。进程退出后的 replacement 由 manifest/checkpoint
识别，本任务不把 exact-byte path read 冒充永久对象身份。
...
`std::os::windows::fs::OpenOptionsExt::share_mode`，不引入 `unsafe`。若未来需要非 Windows hardware runner，
必须先设计等价的 retained-object/no-replace primitive；不得静默使用只按路径复查的弱化 fallback。测试只运行
unknown command、未授权 Amiibo 或直接调用 reservation API，不枚举/打开串口。
...
约束。这些不进入 sequence timing 或 latency sample。目录规模按设计接近常数；发现额外 entry 立即失败。
...
10. unsafe command 不能在目录内外创建 tombstone、final 或 staging。
11. 默认目录时间碰撞时，directory-wide create_new 仍只允许一个进程进入 dispatch。
12. reservation JSON 只含 relative names；lease-specific staging、auxiliary planning 和 envelope字段完整、可从磁盘
    重读。进程退出后篡改文件时，后续磁盘 hash 检查必须可报告 mismatch，不宣称 reservation 自身防篡改。
...
- 把 transaction 期间的 share-mode object lock 描述成进程退出后的永久 tamper protection；
