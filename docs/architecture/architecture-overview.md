...
**[已决定]** EasyCon SDK v1 是用户进程内加载的共享核心，不要求启动额外进程，也不定义网络协议。四种语言只做类型、异步和生命周期适配，Runtime、Controller、Vision 的行为由同一 Rust 实现决定。
...
5. 用户通过宿主语言显式观察完成、取消和关闭；析构只作为最后兜底。
...
flowchart TB
    ...
    subgraph Maintenance["独立维护资产，非 v1 bundle"]
        ECS["easycon-ecs + ECS spec/fixture/conformance"]
    end
```

箭头只表示 v1 的编译期依赖。`easycon-ecs` 及相关规范资产保留为 dormant workspace maintenance，但没有箭头连入
`easycon-sdk`、公共 C ABI 或 canonical native bundle。
...
- 不知道按钮、Vision 算法细节、OpenCV 或具体串口协议。
...
- 唯一聚合根：创建并连接 Runtime、Controller 和 Vision。
- 执行资源树、Controller 写 lease、ActionSequence 与关闭顺序等跨域规则。
...
- Runtime、Controller 或 Vision 公共状态机。
...
| .NET | `LibraryImport`/PInvoke + `SafeHandle` | C++/CLI、复制 Controller 或 Vision 逻辑 |
...
sequenceDiagram
    ...
    L->>A: submit label/template/OCR operation
    A->>V: validate handle and enqueue operation
    V->>F: acquire snapshot reference
    V->>N: match borrowed pixels + label target
    N-->>V: position + native score
    V-->>A: normalized 0.0..1.0 result
    A-->>L: operation result/event observation
```
...
2. `easycon-runtime` 不依赖 Controller、Vision、serial 或 native。
3. `easycon-controller` 不依赖 serial 的系统实现。
4. 只有 `easycon-vision` 可以依赖 `easycon-native-sys`；只有 `easycon-native-sys` 可以 include 私有 bridge header。
5. `easycon-sdk` 不依赖 `easycon-ecs` 或 ECS port；`easycon-ecs` 是不进入 v1 bundle 的独立维护资产。
...
- event read/observation timeout 不等于 operation deadline，也不隐式取消 operation。
...
- ActionSequence 的取消、失败和 close 均在 Controller lease 释放及中立化/stream settlement 后才完成。
